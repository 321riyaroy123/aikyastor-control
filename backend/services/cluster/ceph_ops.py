"""
services/cluster/ceph_ops.py - Ceph cluster operations
Handles stats, health checks, and cluster queries

Moved from: ceph_ops.py (project root)
Responsibility: unchanged — runs `ceph` CLI commands (df, health, version)
and is imported by block_storage.py, object_storage.py, and
cluster_routes.py. Also the sole owner of run_ceph_cmd(), the shared
subprocess wrapper used by every other service module.
"""
...
from config.config import CMD_TIMEOUT, CEPH_CONF
