...
def scrape_host_metrics(ssh_client):
    """
    Gathers custom system and Ceph CLI performance snapshots via a single
    atomic SSH command.
    """
    custom_metrics = {}
    ...
    try:
        ...
        stdin.close()  # No password to send -- see docstring above.
...
def _ensure_ssh_connected(ssh):
    """
    Check whether the given SSHClient's connection is still alive.
    """
    return ceph_ai_ssh.is_connection_alive(ssh)
...
def main():
    ...
    try:
        ssh = ceph_ai_ssh.connect()
    except ceph_ai_ssh.CephAISSHConfigError as e:
        print(f"[metrics_collector] SSH configuration error: {e}", flush=True)
        print("[metrics_collector] Continuing with Netdata-HTTP-only collection.", flush=True)
        ssh = None
    except Exception as e:
        print(f"[metrics_collector] Initial SSH connection failed: {e}", flush=True)
        ssh = None
    ...
    while True:
        ...
        if now - last_ssh_check >= 6:
            if ssh is None or not _ensure_ssh_connected(ssh):
                try:
                    ssh = ceph_ai_ssh.connect()
                except Exception as e:
                    print(f"[metrics_collector] SSH reconnect failed: {e}", flush=True)
                    ssh = None
